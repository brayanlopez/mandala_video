...
import { computeLayout, sanitizePath, resolveImage, computeMandalaLayout } from '../js/geometry.js'
...
describe('sanitizePath', () => {
  ...
  test('no modifica rutas relativas seguras', () => {
    expect(sanitizePath('images/ring1/frutilla.png')).toBe('images/ring1/frutilla.png')
    expect(sanitizePath('center/flor_central.png')).toBe('center/flor_central.png')
  })
  ...
  test('elimina segmentos . actuales', () => {
    expect(sanitizePath('./images/./ring1/file.png')).toBe('images/ring1/file.png')
    expect(sanitizePath('a/./b/./c.png')).toBe('a/b/c.png')
  })
  ...
  test('permite guiones bajos, guiones y puntos en nombres de archivo', () => {
    expect(sanitizePath('images/ring-1/flor_pequena.png')).toBe('images/ring-1/flor_pequena.png')
...
describe('computeMandalaLayout', () => {
  ...
  test('entranceOrder es un índice global secuencial (0, 1, 2, ...)', () => {
    const config = makeConfig([
      ...
      { count: 3, radius: 200, imgSize: 80, images: ['c.png', 'd.png', 'e.png'] },
    ])
  ...
  test('cada slot contiene todos los campos del tipo MandalaSlot', () => {
    ...
    const requiredFields = ['x', 'y', 'angleDeg', 'ring', 'slotIndex', 'imgSize', 'imageSrc', 'entranceOrder']
    computeMandalaLayout(config).forEach((slot) => {
